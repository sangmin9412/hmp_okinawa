  ...
  ({ children, className, label, required, labelClassName, names = [] }: FormRowProps, ref) => {
    ...
    return (
      <div
        ...
        className={cn(
          "flex flex-col gap-[.8rem] lg:flex-row lg:gap-0 overflow-hidden outline-none focus:ring-4",
          className
        )}
      >
        <div className={cn("py-[.9rem] lg:pb-0 flex-grow-0 flex-shrink-0 w-full lg:w-[28.2rem]", labelClassName)}>
          <label className={cn("text-md leading-[2.2rem]", errorLabelClass)}>{label}</label>
          {required && (
            <span className='inline ml-[.4rem]'>
              <svg
                className='inline w-[6px] h-[22px]'
                width='6'
                height='22'
                viewBox='0 0 6 22'
                fill='none'
