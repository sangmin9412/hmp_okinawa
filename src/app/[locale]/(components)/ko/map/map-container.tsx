...
export const MapContainerKO = () => {
  const [currentTab, setCurrentTab] = useState<number>(1);

  useEffect(() => {
    const images = [
      getImagePath("/images/ev_map_01_map_pc.png", false),
      getImagePath("/images/ev_map_01_pin_v2_pc.png", false),
      getImagePath("/images/ev_map_01_map_mo.png", false),
      getImagePath("/images/ev_map_01_pin_mo.png", false),
      getImagePath("/images/map_bg_pc.png", false),
      getImagePath("/images/map_bg_mo.png", false)
    ];
...
const TabButtons = ({ currentTab, onChangeTab }: { currentTab: number; onChangeTab: (tab: number) => void }) => {
  return (
    <div className='flex relative lg:absolute top-0 right-0 rounded-[5.6rem] bg-white/40 p-[.4rem] mb-[1.6rem] lg:mb-0'>
      <button
        className={`flex-1 lg:flex-auto flex items-center justify-center lg:px-[3.2rem] h-[3.6rem] lg:h-[4.8rem] rounded-[4.8rem] text-[1.3rem] lg:text-[1.6rem] leading-[2.6rem] text-[#fff] font-[400] ${currentTab === 1 ? "font-[500] bg-primary" : ""}`}
        onClick={() => onChangeTab(1)}
      >
        <span>오키나와 추천 명소</span>
      </button>
      <button
        className={`flex-1 lg:flex-auto flex items-center justify-center lg:px-[3.2rem] h-[3.6rem] lg:h-[4.8rem] rounded-[4.8rem] text-[1.3rem] lg:text-[1.6rem] leading-[2.6rem] text-[#fff] font-[400] ${currentTab === 2 ? "font-[500] bg-primary" : ""}`}
...
const EvMap = ({
  ...
  return (
    <div className={cn("relative lg:py-[12rem] py-[5.6rem] bg-[#002C5F]", className)}>
      <div className='container'>
        <div className='relative pb-[6.4rem] lg:text-[4rem] text-[2.4rem] lg:leading-[1.6] leading-[1.5] font-[600]'>
          <h2 className='text-[#0793EA] font-[500]'>현대 EV와 함께,</h2>
          <h2 className='mb-[1.6rem] text-[#fff] font-[500]'>충전 걱정 없는 편안한 여행을 떠나보세요.</h2>
          <TabButtons currentTab={currentTab} onChangeTab={onChangeTab} />
          <h3 className='lg:text-[1.8rem] text-[1.5rem] leading-[1.6] font-[400] text-[#fff]'>
            IONIQ 5(485km)와 KONA Electric(417km)의 여유로운 항속거리로 충전 걱정 없는 오키나와 여행을 즐겨보세요.
            <br />
            <br className='mo-only' />
            현대 모빌리티 패스포트 고객에게는 충전소 안내와 이용 방법이 상세히 제공되며, 일본은 전기차 이용자가 적어
            충전소가 여유롭습니다.
            <br />
            <br className='mo-only' />
            또한, 완충 조건 없이 편리하게 렌터카를 반납할 수 있어 여행의 끝까지 스트레스 없이 마무리할 수 있습니다.
            <br />
            <br className='mo-only' />
            현대 EV가 캐리하는 오키나와 여행, 현대 모빌리티 패스포트로 경험해보세요.
          </h3>
        </div>
        <div>
          <div className='relative w-100%'>
            <Image
              className='hidden lg:block ml-[17.5rem]'
              src={getImagePath("/images/ev_map_01_map_pc.png", false)}
              width={930}
              height={1200}
              alt='EV MAP'
              unoptimized={true}
            ...
            />
            <Image
              className='hidden lg:block absolute top-[18.5rem] right-[18rem]'
              src={getImagePath("/images/ev_map_01_pin_v2_pc.png", false)}
              width={763}
              height={876}
              alt='EV PIN'
              unoptimized={true}
            ...
            <Image
              ...
              src={getImagePath("/images/ev_map_01_map_mo.png", false)}
              width={320}
              height={412}
              alt='EV MAP'
              unoptimized={true}
            ...
            />
            <Image
              className='block lg:hidden absolute top-0 w-full h-auto'
              src={getImagePath("/images/ev_map_01_pin_mo.png", false)}
              width={320}
              height={412}
              alt='EV PIN'
              unoptimized={true}
              priority={true}
            />
            <div className='relative lg:absolute right-0 bottom-0 mt-[4.8rem] lg:mt-0'>
              <Image
                src={getImagePath("/images/ev_map_01_info_pc.png")}
                width={640}
                height={109}
                alt='EV MAP'
                className='hidden lg:block w-auto h-[10rem]'
              />
              <Image
                src={getImagePath("/images/ev_map_01_info_mo.png")}
                width={320}
                height={168}
                alt='EV MAP'
                className='block lg:hidden w-full h-auto'
              />
              <div className='lg:mt-[1.2rem] mt-[0.8rem]'>
                <DotList
                  className='after:bg-[#fff] after:opacity-40'
                  items={[
                    <span
                      key={"ev_map_notice"}
                      className='text-[#fff] opacity-40'
                      dangerouslySetInnerHTML={{
                        __html: t.raw("ev_map_notice")
                      }}
                    />
                  ]}
                />
      ...
      <div className='container'>
        ...
        <div className='relative w-100%'>
          <Image
            ...
            src={getImagePath("/images/map_bg_pc.png", false)}
            width={930}
            height={1200}
            alt='OKINAWA'
            unoptimized={true}
          ...
          <Image
            ...
            src={getImagePath("/images/map_bg_mo.png", false)}
            width={320}
            height={412}
            alt='OKINAWA'
            unoptimized={true}
